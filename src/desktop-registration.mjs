...
const serverName = 'whatsapp-manutencao';
...
export function registrationText(original, executable) {
  ...
  if (config.mcp_servers?.[serverName] && !range) throw new Error('Já existe uma conexão WhatsApp com configuração própria. Ela foi preservada.');
  ...
  const omit=c=>{const copy=structuredClone(c);if(copy.mcp_servers){delete copy.mcp_servers[serverName];if(!Object.keys(copy.mcp_servers).length)delete copy.mcp_servers;}return copy;};
...
export function registerLocal(executable,file=configPath()) {
  ...
  if (current.mcp_servers?.[serverName]?.command===executable &&
      JSON.stringify(current.mcp_servers[serverName].args)===JSON.stringify(['--mcp']) && current.mcp_servers[serverName].enabled===true && managedRegion(original))
...
export function registrationStatus(executable,file=configPath()) {
  try {const c=parse(read(file).replace(/^\uFEFF/,''));const item=c.mcp_servers?.[serverName];return item?.command===executable && item.enabled===true && JSON.stringify(item.args)===JSON.stringify(['--mcp']);}
  catch {return false;}
}
